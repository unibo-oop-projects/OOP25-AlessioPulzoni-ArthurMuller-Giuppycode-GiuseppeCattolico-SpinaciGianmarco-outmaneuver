...
class EntityControllerImplTest {
    ...
    private static final class TestMissile extends MissileImpl {
        TestMissile(final Vector2 pos, final double dir, final double speed) {
            super(pos, speed, 0, 8.0, -1, 0, 0);
            setVelocity(Vector2.fromAngle(dir).scale(speed));
        }
    ...
    void spawnMissile_addsToEntities() {
        final TestMissile missile = new TestMissile(Vector2.ZERO, 0, 100);
    ...
    void clearAll_removesMissiles() {
        final TestMissile missile = new TestMissile(Vector2.ZERO, 0, 0);
        entityCtrl.spawnMissile(missile);
